    ...
    <Section background="default" padding="lg">
      <div className="layout-container">
        <motion.div className="mb-12 text-left" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
          ...
          <h2 className="text-display mt-4 text-3xl font-semibold tracking-tight text-ink sm:text-4xl md:text-5xl">{heading}</h2>
          <p className="mt-4 max-w-3xl text-base text-ink-soft sm:text-lg">{intro}</p>
        </motion.div>

        <motion.div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-2" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} role="list">
            ...
            >
              <Card className="group relative flex h-full flex-col overflow-hidden rounded-xl border border-border/60 bg-surface-1/50 shadow-soft backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-violet/30 hover:shadow-violet-glow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet/60 focus-visible:ring-offset-2 focus-visible:ring-offset-surface-0">
                ...
                <CardContent className="relative flex flex-1 flex-col gap-4 p-6">
                  <h3 className="text-base font-semibold text-ink">{option.option}</h3>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-sm leading-relaxed text-ink-soft">{option.whereWorks}</span>
                  </div>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-sm leading-relaxed text-ink-soft">{option.whereFails}</span>
                  </div>
                  ...
                  <div className="mt-1 flex items-start gap-2 rounded-xl border border-violet/20 bg-violet/10 p-3">
                    ...
                    <span className="text-sm font-medium leading-relaxed text-violet-soft">{option.ourPosition}</span>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
