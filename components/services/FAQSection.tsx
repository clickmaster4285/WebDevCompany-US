    ...
    <Section background="background" padding="lg">
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="text-display mt-5 text-3xl font-semibold text-foreground md:text-4xl">
        ...
        <motion.div className="space-y-4" variants={containerVariants} initial={shouldReduceMotion ? false : "hidden"} whileInView="visible" viewport={{ once: true, margin: "-50px" }}>
          {faqs.map((faq, index) => {
              ...
              <motion.div key={index} variants={activeItemVariants} custom={index}>
                <div className={`group relative overflow-hidden rounded-2xl border bg-background transition-all duration-300 ${isOpen ? "border-violet/40 shadow-lg shadow-violet/5" : "border-border/60 hover:border-violet/30"}`}>
                  ...
                  <button className={`flex w-full items-center gap-4 px-6 py-5 text-left transition-colors duration-300 md:px-7 md:py-6 ${isOpen ? "bg-violet/[0.03]" : "hover:bg-muted/30"}`} onClick={() => toggle(index)} aria-expanded={isOpen}>
                    ...
                    <span className="flex-1 text-base font-semibold text-foreground md:text-lg">{faq.question}</span>
                    <motion.span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition-colors duration-300 ${isOpen ? "bg-violet text-foreground" : "bg-violet/10 text-violet group-hover:bg-violet group-hover:text-foreground"}`} animate={{ rotate: isOpen ? 180 : 0 }} transition={{ duration: 0.25 }}>
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
