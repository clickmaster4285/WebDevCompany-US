...
export function TechnologySubPagesSidebar({ parent, subPages }: Props) {
  ...
  return (
    <aside className="hidden lg:block">
      <div className="sticky top-20 max-h-[calc(100vh-6rem)] overflow-y-auto rounded-[1.35rem] border border-slate-200 bg-white/90 p-5 shadow-[0_18px_60px_rgba(15,23,42,0.07)] backdrop-blur-xl">
        {/* Header */}
        ...
        <div className="mt-3 border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2">
            <span className="text-lg">{parent.icon || "⚙️"}</span>
            <div>
          ...
          {subPages.map((subPage) => {
            ...
            return (
              ...
              >
                ...
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-slate-200 bg-white text-sm shadow-sm transition-colors group-hover:border-violet/30 group-hover:bg-violet/5">
                  {subPage.icon || parent.icon || "⚙️"}
                </span>

                <div className="flex-1 min-w-0">
                  <p
                    className={`text-sm font-semibold leading-tight transition-colors line-clamp-2 ${
